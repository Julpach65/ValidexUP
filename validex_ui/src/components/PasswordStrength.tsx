    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-xs">
            {requirements.map(req => {
                ...
                return (
                    <div key={req.id} className={`flex items-center gap-2.5 transition-colors duration-300 ${isValid ? 'text-emerald-400' : 'text-slate-500'}`}>
                        <span
                            key={`${req.id}-${isValid}`} // Forza el re-renderizado para la animación
                            className="material-icons-round text-base animate-in zoom-in duration-300"
                        >
                            {isValid ? 'check_circle' : 'highlight_off'}
                        </span>
